(() => {
  ...
  function populateTypes(){const s=$('signType');s.innerHTML=TYPES.map(x=>`<option>${x}</option>`).join('')}
  function bind(){
    ...
    $('backBtn').onclick=()=>history.length>1?history.back():newSign();
    setupOverlayEditing();
  ...
  async function init(){
    ...
    startGPS();refreshMetricCapability();
    setTimeout(prewarmOCR,250);
    if('serviceWorker'in navigator)navigator.serviceWorker.register('./service-worker.js?b=12.0.0',{scope:'./',updateViaCache:'none'}).catch(()=>{});
    pill($('systemStatus'),'Saha hazır','');
  }

  async function prewarmOCR(){
    const out=$('ocrReady');out.textContent='Hazırlanıyor…';
    try{if(window.TabelaFastOCR?.prewarm){const ok=await TabelaFastOCR.prewarm();out.textContent=ok?'Türkçe hazır':'Yedek mod';}else out.textContent=window.Tesseract?'Türkçe hazır':'Yüklenemedi';}
    catch(e){console.warn(e);out.textContent='İlk okumada yükle'}
  }

  function startGPS(){
    if(!navigator.geolocation){$('gpsBadge').textContent='GPS yok';return}
    navigator.geolocation.watchPosition(p=>{state.gps={lat:p.coords.latitude,lng:p.coords.longitude,accuracy:p.coords.accuracy,at:Date.now()};$('gpsBadge').textContent=`GPS ±${Math.round(p.coords.accuracy)} m`;$('locationText').textContent=`${p.coords.latitude.toFixed(6)}, ${p.coords.longitude.toFixed(6)} • ±${Math.round(p.coords.accuracy)} m`;},()=>{$('gpsBadge').textContent='GPS izni yok';$('locationText').textContent='Konum alınamadı';},{enableHighAccuracy:true,maximumAge:5000,timeout:12000});
  }

  function refreshMetricCapability(){
    let cap={available:false,platform:'web'};try{cap=window.TabelaMetric?.capability?.()||cap}catch{}
    if(cap.available){$('metricBadge').textContent=cap.platform==='ios'?(cap.caps?.lidar?'iOS • LiDAR hazır':'iOS • ARKit hazır'):'Android • ARCore hazır';$('metricSource').textContent='Gerçek 3B sensör köprüsü hazır.';pill($('metricStatus'),'3B HAZIR','');}
    else{$('metricBadge').textContent='WEB • metre kapalı';$('metricSource').textContent='Saf web tarayıcıda gerçek metre kaynağı yok. Pikselden metre üretilmez.';pill($('metricStatus'),'ÖLÇÜ YOK','warn')}
  }
  ...
  async function startCamera(){
    ...
    try{
      ...
      state.stream=stream;state.mode='live';state.liveStartedAt=Date.now();
      const v=$('video');v.srcObject=stream;v.classList.remove('hidden');$('photoCanvas').classList.add('hidden');$('scanner').classList.remove('hidden');$('retakeBtn').classList.add('hidden');$('captureBtn').disabled=true;$('captureBtn').textContent='📸 Fotoğraf Çek';
      await new Promise(resolve=>{if(v.readyState>=2)return resolve();v.onloadedmetadata=()=>resolve()});await v.play();
      setFrameRatio(v.videoWidth,v.videoHeight);syncOverlay(v.videoWidth,v.videoHeight);$('detectBadge').textContent='Tabela aranıyor…';liveTick();
  ...
  function frameToWork(){
    const v=$('video'),maxW=360,scale=Math.min(1,maxW/Math.max(1,v.videoWidth)),w=Math.max(1,Math.round(v.videoWidth*scale)),h=Math.max(1,Math.round(v.videoHeight*scale));workCanvas.width=w;workCanvas.height=h;workCtx.drawImage(v,0,0,w,h);return{canvas:workCanvas,scaleBack:v.videoWidth/w};
  }
  ...
  function detectCandidate(canvas){
    const w=canvas.width,h=canvas.height,ctx=canvas.getContext('2d',{willReadFrequently:true}),d=ctx.getImageData(0,0,w,h).data,n=w*h,gray=new Float32Array(n),grad=new Float32Array(n),sq=new Float32Array(n);
    for(let i=0,p=0;i<n;i++,p+=4){const g=.299*d[p]+.587*d[p+1]+.114*d[p+2];gray[i]=g;sq[i]=g*g}
    for(let y=1;y<h-1;y++)for(let x=1;x<w-1;x++){const i=y*w+x,gx=Math.abs(gray[i+1]-gray[i-1]),gy=Math.abs(gray[i+w]-gray[i-w]);grad[i]=Math.min(255,gx+gy)}
    const IG=makeIntegral(grad,w,h),IL=makeIntegral(gray,w,h),IS=makeIntegral(sq,w,h);let best=null;
    const wr=[.28,.38,.5,.64,.78],hr=[.12,.18,.25,.34,.44];
    for(const wf of wr)for(const hf of hr){const rw=Math.round(w*wf),rh=Math.round(h*hf),area=rw*rh,frac=area/n,aspect=rw/rh;if(frac<.045||frac>.62||aspect<.45||aspect>7)continue;const sx=Math.max(8,Math.round(rw*.18)),sy=Math.max(7,Math.round(rh*.22));for(let y=0;y+rh<=h;y+=sy)for(let x=0;x+rw<=w;x+=sx){const g=rectSum(IG,w,x,y,rw,rh)/area,l=rectSum(IL,w,x,y,rw,rh)/area,l2=rectSum(IS,w,x,y,rw,rh)/area,std=Math.sqrt(Math.max(0,l2-l*l));const t=Math.max(2,Math.round(Math.min(rw,rh)*.06)),per=(rectSum(IG,w,x,y,rw,t)+rectSum(IG,w,x,y+rh-t,rw,t)+rectSum(IG,w,x,y,t,rh)+rectSum(IG,w,x+rw-t,y,t,rh))/Math.max(1,2*rw*t+2*rh*t);const cx=(x+rw/2)/w,cy=(y+rh/2)/h,centerPenalty=(Math.abs(cx-.5)+Math.abs(cy-.48))*7,aspectBonus=aspect>=1.3&&aspect<=5?7:2,score=g*.66+std*.34+per*.24+aspectBonus-centerPenalty;if(!best||score>best.score)best={x,y,w:rw,h:rh,score}}}
    if(!best||best.score<30)return null;best.confidence=Math.round(clamp((best.score-20)*2.25,45,96));return best;
  }
  ...
  function liveTick(){
    ...
    try{const {canvas,scaleBack}=frameToWork(),c=detectCandidate(canvas);if(c){const mapped={x:c.x*scaleBack,y:c.y*scaleBack,w:c.w*scaleBack,h:c.h*scaleBack,confidence:c.confidence};if(iou(mapped,state.stableBBox)>.55)state.stableCount++;else state.stableCount=1;state.stableBBox=mapped;if(state.stableCount>=2){state.bbox=mapped;$('captureBtn').disabled=false;$('detectBadge').textContent=`✓ Tabela adayı bulundu • %${mapped.confidence}`;drawOverlay()}else $('detectBadge').textContent='Tabela doğrulanıyor…';}else{state.stableCount=0;state.stableBBox=null;if(Date.now()-state.liveStartedAt>4500){$('captureBtn').disabled=false;$('captureBtn').textContent='📸 Fotoğraf Çek & Alan Seç';$('detectBadge').textContent='Otomatik aday bulunamadı • çekim sonrası alanı ayarlayın'}else $('detectBadge').textContent='Tabela aranıyor…';}}
    catch(e){console.warn('live detect',e)}
    state.liveTimer=setTimeout(liveTick,650);
  }

  async function loadFile(e){const f=e.target.files?.[0];if(!f)return;closeStream();resetAnalysis();const data=await fileData(f),img=await imageFrom(data),c=$('photoCanvas');c.width=img.naturalWidth;c.height=img.naturalHeight;c.getContext('2d').drawImage(img,0,0);state.capturedDataUrl=c.toDataURL('image/jpeg',.94);state.mode='captured';setFrameRatio(c.width,c.height);syncOverlay(c.width,c.height);const small=resizeCanvas(c,360),cand=detectCandidate(small.canvas);state.bbox=cand?{x:cand.x*small.scaleBack,y:cand.y*small.scaleBack,w:cand.w*small.scaleBack,h:cand.h*small.scaleBack,confidence:cand.confidence}:defaultBBox(c.width,c.height);$('scanner').classList.remove('hidden');$('video').classList.add('hidden');c.classList.remove('hidden');$('captureBtn').classList.add('hidden');$('retakeBtn').classList.remove('hidden');$('detectBadge').textContent=cand?`✓ Tabela alanı bulundu • %${cand.confidence}`:'Alanı yeşil çerçeveyle düzeltin';drawOverlay();afterCapture()}
  ...
  function defaultBBox(w,h){return{x:w*.15,y:h*.27,w:w*.70,h:h*.32,confidence:0,manual:true}}

  async function capture(){
    if(state.mode!=='live')return;const v=$('video'),c=$('photoCanvas');c.width=v.videoWidth;c.height=v.videoHeight;c.getContext('2d').drawImage(v,0,0,c.width,c.height);state.capturedDataUrl=c.toDataURL('image/jpeg',.94);state.mode='captured';if(!state.bbox)state.bbox=defaultBBox(c.width,c.height);closeStream();v.classList.add('hidden');c.classList.remove('hidden');$('captureBtn').classList.add('hidden');$('retakeBtn').classList.remove('hidden');$('detectBadge').textContent=state.bbox.manual?'Alanı yeşil çerçeveyle düzeltin':'✓ Tabela bulundu • alanı kontrol edin';syncOverlay(c.width,c.height);drawOverlay();afterCapture();
  }
  function retake(){if($('fileInput').value){$('fileInput').value='';newSign()}else startCamera()}
  function resetAnalysis(){state.capturedDataUrl=null;state.bbox=null;state.stableBBox=null;state.stableCount=0;state.ocr=null;state.metric=null;$('resultCard').classList.add('hidden');$('ocrText').value='';renderMetric()}
  ...
  async function afterCapture(){
    $('resultCard').classList.remove('hidden');$('resultStatus').textContent='Tabela alanı bulundu. OCR ve ölçüm sağlayıcısı kontrol ediliyor.';inferType();requestMetric();await runOCR();$('resultCard').scrollIntoView({behavior:'smooth',block:'start'});
  }
  ...
  function inferType(){const b=state.bbox,c=$('photoCanvas'),r=b?b.w/Math.max(1,b.h):2,area=b?b.w*b.h/Math.max(1,c.width*c.height):0;let type='Dükkan / mağaza tabelası';if(r>4.2&&area>.16)type='Bina cephe tabelası';else if(r<.75&&area>.12)type='Totem';else if(area>.38)type='Billboard / büyük pano';$('signType').value=type}
  ...
  async function runOCR(){
    ...
    try{
      if(!window.TabelaFastOCR)throw new Error('Türkçe OCR motoru yüklenemedi');
      const shape={bbox:{x:state.bbox.x,y:state.bbox.y,w:state.bbox.w,h:state.bbox.h}};
      const r=await TabelaFastOCR.run(state.capturedDataUrl,shape,(pct,status)=>{$('ocrStatus').textContent=status||`OCR %${pct}`},3000);state.ocr=r;const text=cleanText(r.text);$('ocrText').value=text;$('ocrTime').textContent=`${r.elapsedMs||0} ms`;
      if(text){$('ocrStatus').textContent=r.validated?'OCR yüksek güven':'OCR kontrol gerekli';pill($('qualityBadge'),`OCR %${r.confidence||0}`,r.validated?'':'warn');analyzeCompany(text)}
      else{$('ocrStatus').textContent='Metin okunamadı • alanı düzeltip tekrar deneyin';pill($('qualityBadge'),'OCR YOK','warn');analyzeCompany('')}
    ...
    finally{runOCR.busy=false;$('reanalyzeBtn').disabled=false}
  }
  ...
  function findDuplicate(text){if(!state.gps)return null;return state.records.find(r=>distanceM(state.gps,r.gps)<25&&textSimilarity(text,r.ocrText)>.62)}
  ...
  async function saveRecord(){
    const text=cleanText($('ocrText').value),dup=findDuplicate(text);if(dup&&!confirm(`Yakında benzer kayıt var: ${dup.companyName||dup.ocrText||dup.id}. Yine de kaydedilsin mi?`))return;
    const rec={id:'TBL-'+Date.now().toString(36).toUpperCase(),createdAt:new Date().toISOString(),project:$('projectName').value.trim()||'SAHA-001',signType:$('signType').value,ocrText:text,ocrConfidence:Number(state.ocr?.confidence)||0,ocrEngine:state.ocr?.engine||'manual',companyName:$('companyName').textContent,sector:$('sectorText').textContent.replace(/^Sektör:\s*/,''),bbox:state.bbox?{x:state.bbox.x,y:state.bbox.y,w:state.bbox.w,h:state.bbox.h}:null,gps:state.gps?{lat:state.gps.lat,lng:state.gps.lng,accuracy:state.gps.accuracy}:null,measurement:state.metric?{...state.metric}:null,thumbnail:await makeThumb()};
    try{await dbPut(rec);state.records.unshift(rec);renderSummary();renderRecords();toast('Tabela kaydedildi ✓');newSign()}catch(e){console.error(e);toast('Kayıt yapılamadı')}
  }
  ...
  function renderRecords(){const box=$('recordsList');const rs=[...state.records].sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt))).slice(0,12);if(!rs.length){$('recordsCard').classList.add('hidden');box.innerHTML='';return}$('recordsCard').classList.remove('hidden');box.innerHTML=rs.map(r=>`<div class="record-row"><strong>${escapeHtml(r.companyName||r.ocrText||r.id)}</strong><small>${escapeHtml(r.signType||'')} • ${new Date(r.createdAt).toLocaleString('tr-TR')}${r.measurement?.area?` • ${Number(r.measurement.area).toFixed(2)} m²`:''}</small></div>`).join('')}
