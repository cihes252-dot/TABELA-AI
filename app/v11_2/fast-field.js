(() => {
  ...
  function mapSegToFrame(seg,w,h){
    const n=normalizedBox(seg);if(!n||!w||!h)return null;
    return{...seg,bbox:{x:n.x*w,y:n.y*h,w:n.w*w,h:n.h*h},__frameWidth:w,__frameHeight:h,boundaryDetected:true,boundarySource:'stable-live-roi'};
  }
  function normalizeShape(seg){
    if(!seg?.bbox)return seg;const out={...seg,debug:{...(seg.debug||{})}},aspect=Number(seg.bbox.w)/Math.max(1,Number(seg.bbox.h)),ellipseStrong=seg.debug?.ellipseStrong===true;
    if((out.shapeType==='oval'||out.shapeType==='circle')&&!ellipseStrong){
      if(aspect>=1.28){out.shapeType='horizontal-rectangle';out.shapeLabel='Yatay dikdörtgen';out.boundarySource=(out.boundarySource||'geometry')+'+rect-guard'}
      else if(aspect<=.78){out.shapeType='vertical-rectangle';out.shapeLabel='Dikey dikdörtgen';out.boundarySource=(out.boundarySource||'geometry')+'+rect-guard'}
    }
    return out;
  }
  function selectPhotoSeg(photoSeg,frame){
    const stableFresh=live.preCaptureSeg&&(Date.now()-live.lastDetectedAt)<2600,pre=stableFresh?mapSegToFrame(live.preCaptureSeg,frame.width,frame.height):null;
    const photoValid=photoSeg?.boundaryDetected!==false&&Number(photoSeg?.confidence)>=52;
    if(!pre)return{seg:normalizeShape(photoSeg),preMatch:0,usedStable:false};
    const preNorm=normalizedBox(pre),photoNorm=normalizedBox(photoSeg),match=photoValid?iouNorm(preNorm,photoNorm):0;
    if(!photoValid||match<.45){const s=normalizeShape({...pre,confidence:Math.max(58,Math.min(86,Number(pre.confidence)||65)),mode:'stable-live-roi'});return{seg:s,preMatch:match,usedStable:true}}
    const a=photoSeg.bbox,b=pre.bbox,k=.58,fused={x:a.x*k+b.x*(1-k),y:a.y*k+b.y*(1-k),w:a.w*k+b.w*(1-k),h:a.h*k+b.h*(1-k)};
    const s=normalizeShape({...photoSeg,bbox:fused,__frameWidth:frame.width,__frameHeight:frame.height,boundarySource:(photoSeg.boundarySource||photoSeg.mode||'geometry')+'+stable-fused'});
    return{seg:s,preMatch:match,usedStable:true};
  }
  function mountLiveUI(){
    ...
    const hint=document.querySelector('.hint');if(hint)hint.textContent='OTOMATİK ÖN TESPİT → SEN FOTOĞRAF ÇEK → TÜRKÇE OCR';
  ...
  function showLiveBoundary(seg){const b=normalizedBox(seg),box=$('liveBoundaryBox');if(!b||!box)return;box.style.display='block';box.style.left=(b.x*100).toFixed(2)+'%';box.style.top=(b.y*100).toFixed(2)+'%';box.style.width=(b.w*100).toFixed(2)+'%';box.style.height=(b.h*100).toFixed(2)+'%'}
  async function previewType(data,seg){try{const r=await window.TabelaSignTaxonomy?.classify?.(data,{shape:seg});if(r){live.preCaptureType=r;return r}}catch{}return null}
  function bboxMetricMeta(seg){const b=seg?.bbox,n=normalizedBox(seg),w=Number(seg?.__frameWidth||photo?.naturalWidth||photo?.width||video?.videoWidth||0),h=Number(seg?.__frameHeight||photo?.naturalHeight||photo?.height||video?.videoHeight||0);if(!b||!n||!w||!h)return null;return{captureWidth:w,captureHeight:h,bboxNormalized:n,pixelWidth:Math.round(b.w),pixelHeight:Math.round(b.h),frameWidthPct:Math.round(n.w*100),frameHeightPct:Math.round(n.h*100)}}
  function showVisualBoundary(seg){const m=bboxMetricMeta(seg),badge=$('liveMetricBadge');if(!m||!badge)return m;badge.className='badge warn';badge.textContent=`GÖRSEL SINIR • ${m.pixelWidth}×${m.pixelHeight} px • kadraj %${m.frameWidthPct}×%${m.frameHeightPct}`;return m}
  ...
  async function liveTick(){
    ...
    if(!live.busy&&v&&!v.classList.contains('hidden')&&v.readyState>=2&&v.videoWidth>0){
      live.busy=true;
      try{
        const frame=snapFrame(v,640,.70),raw=await window.TabelaSegmentation.detect(frame.data),seg=normalizeShape(raw);seg.__frameWidth=frame.width;seg.__frameHeight=frame.height;
        if(seg?.boundaryDetected!==false&&Number(seg?.confidence)>=55){
          ...
          if(live.stable>=2){live.preCaptureSeg=seg;showLiveBoundary(seg);const type=await previewType(frame.data,seg),key=candidateKey(seg),badge=$('liveDetectBadge');live.candidateKey=key;if(badge){badge.className='badge ok';badge.textContent=`TABELA HAZIR ✓ ${type?.label||seg.shapeLabel} • sınır %${seg.confidence} • FOTOĞRAF ÇEK`}showVisualBoundary(seg);if(live.autoMetric&&key&&live.metricCandidateKey!==key)requestAutoMetric(seg,{preCapture:true})}
      ...
      }catch{live.stable=0;live.last=null;hideLiveBoundary();const badge=$('liveDetectBadge');if(badge){badge.className='badge warn';badge.textContent='OTOMATİK TESPİT: yeniden aranıyor'}}finally{live.busy=false}
    }
  ...
  function resumeLiveSoon(delay=220){setTimeout(()=>{const v=$('video');if(v?.srcObject&&!v.classList.contains('hidden')&&v.videoWidth>0){const b=$('snapBtn');if(b)b.disabled=false;startLive();const s=$('scanStatus');if(s)s.textContent='Kamera hazır • tabela otomatik aranıyor. TABELA HAZIR görünce Fotoğraf Çek.'}},delay)}
  function patchStorageLifecycle(){const storage=window.TabelaStorage;if(!storage?.add||storage.add.__fastFieldLifecycle)return;const original=storage.add.bind(storage);const wrapped=async(record,photoData)=>{const result=await original(record,photoData);currentMeasure=null;live.metricCandidateKey='';live.preCaptureSeg=null;live.preCaptureType=null;hideLiveBoundary();resumeLiveSoon(300);return result};wrapped.__fastFieldLifecycle=true;storage.add=wrapped}
  function renderTypeResult(r){const select=$('signType');if(!r||!select)return;if([...select.options].some(o=>o.value===r.label))select.value=r.label;let box=$('autoTypeBox');if(!box){box=document.createElement('div');box.id='autoTypeBox';box.className='quality';select.closest('.field')?.appendChild(box)}box.innerHTML=`<b>Otomatik tabela tipi: ${esc2(r.label)}</b> <span class="badge ${r.mode==='trained-model'&&r.confidence>=88?'ok':'warn'}">%${r.confidence}</span><div class="muted">${r.mode==='trained-model'?'eğitimli model':'görsel geometri + sektör tipolojisi • operatör kontrolü'}${r.reasons?.length?' • '+r.reasons.map(esc2).join(' • '):''}</div>`;select.dataset.autoTypeCode=r.code||'';select.dataset.autoTypeConfidence=String(r.confidence||0)}
  async function fastScan(){
    const v=$('video');if(!v||v.classList.contains('hidden')||v.readyState<2){const s=$('scanStatus');if(s)s.textContent='Kamera hazır değil.';return}
    stopLive();duplicateOverride=false;if($('saveBtn'))$('saveBtn').disabled=true;live.metricPending=!!live.metricPending;
    if($('scanStatus'))$('scanStatus').textContent='Fotoğraf alındı • tabela sınırı doğrulanıyor…';stage(1,false,'Tek yüksek çözünürlüklü kare');
    const frame=snapFrame(v,null,.94),data=frame.data;bestData=data;frames=[{data,index:0}];
    let photoSeg=await TabelaSegmentation.detect(data);photoSeg.__frameWidth=frame.width;photoSeg.__frameHeight=frame.height;const selected=selectPhotoSeg(photoSeg,frame),seg=selected.seg,preMatch=selected.preMatch;
    if(!seg||seg.boundaryDetected===false||Number(seg.confidence)<52){if($('scanStatus'))$('scanStatus').textContent='Fotoğrafta güvenilir tabela sınırı bulunamadı. Kadrajı düzeltip tekrar çekin.';stage(2,false,'Tabela sınırı doğrulanmadı');resumeLiveSoon(200);return}
    const q=await TabelaQuality.analyze(data,seg.bbox);currentShape=seg;currentQuality=q;stage(1,q.pass,`Kalite %${q.score}`);const qb=$('qualityBox');if(qb){qb.classList.remove('hidden');qb.innerHTML=`<b>Çekim kalitesi %${q.score}</b><div class="muted">${esc2(q.reasons?.join(' • ')||'Netlik / ışık / kontrast uygun')}</div>`}await draw(data,seg);
    if(!q.pass){if($('scanStatus'))$('scanStatus').textContent='Kalite yetersiz. Tabelaya yaklaşın ve yeniden çekin.';$('retryBtn')?.classList.remove('hidden');resumeLiveSoon(250);return}
    stage(2,true,`${seg.shapeLabel} • sınır %${seg.confidence}${selected.usedStable?' • sabit ROI':''}`);if($('shapeSelect'))$('shapeSelect').value=seg.shapeType;if($('shapeInfo'))$('shapeInfo').textContent=`${seg.shapeLabel} • sınır güven %${seg.confidence} • ${seg.mode||seg.boundarySource||'geometry'}`;showVisualBoundary(seg);
    ...
    stage(3,false,'Türkçe OCR • tek fotoğraf • 3 sn hedef');
    currentOCR=await window.TabelaFastOCR.run(data,seg,(p,s)=>{if($('ocrBar'))$('ocrBar').style.width=p+'%';if($('st3'))$('st3').textContent=s},3000);
    const engine=currentOCR.engine==='apple-vision'?'Apple Vision TR':'Web Türkçe OCR';if($('ocr')){$('ocr').value=currentOCR.text||'';$('ocr').placeholder=currentOCR.text?'OCR sonucu':'Yazı okunamadı — yeniden çek veya manuel gir'}if($('ocrConfidence'))$('ocrConfidence').textContent=`${engine}: %${currentOCR.confidence||0} • ${currentOCR.elapsedMs||0} ms`;if($('ocrBadge')){$('ocrBadge').textContent=currentOCR.validated?'OCR güçlü':'OCR kontrol gerekli';$('ocrBadge').className='badge '+(currentOCR.validated?'ok':'warn')}if($('ocrCandidates'))$('ocrCandidates').innerHTML=currentOCR.text?`<div class="candidate">${esc2(currentOCR.text)} • ${engine} • ${currentOCR.elapsedMs} ms${currentOCR.targetMet?' • hız hedefi ✓':' • 3 sn hedef aşıldı'}</div>`:'<div class="candidate">OCR sonucu yok • yeniden çekin veya metni manuel girin</div>';stage(3,!!currentOCR.text,currentOCR.text?`${currentOCR.text} • ${currentOCR.elapsedMs} ms`:'OCR yeniden çekim / manuel kontrol');
    ...
    if($('scanStatus'))$('scanStatus').textContent=`Fotoğraf tamamlandı • ${engine} ${currentOCR.elapsedMs||0} ms${currentMeasure?.verified?' • gerçek ölçü doğrulandı':metricStarted?' • gerçek ölçüm doğrulanıyor':' • ölçü mevcut değil'}. Sonucu kontrol edip kaydedin.`;$('retryBtn')?.classList.remove('hidden');updateSave();window.dispatchEvent(new CustomEvent('tabela:fast-scan-complete',{detail:{ocr:currentOCR,type,shape:seg,quality:q,metricStarted,preDetectionMatch:preMatch,manualPhoto:true,stableROI:selected.usedStable}}));
  }
  function installMetricLifecycle(){
    window.addEventListener('tabela:measurement',e=>{live.metricPending=false;const d=e.detail||{},badge=$('liveMetricBadge');if(badge){badge.className='badge ok';badge.textContent=`GERÇEK ÖLÇÜ ✓ ${Number(d.widthM||0).toFixed(3)}×${Number(d.heightM||d.diameterM||0).toFixed(3)} m • ${Number(d.areaM2||0).toFixed(3)} m²`}stage(6,true,`${d.source||'3D'} • kalite %${Math.round(Number(d.measurementQualityScore||d.qualityScore||0))}`);if($('scanStatus'))$('scanStatus').textContent=$('photo')?.classList.contains('hidden')?'Ölçüm hazır ✓ Şimdi Fotoğraf Çek + OCR.':'Gerçek ölçüm doğrulandı. OCR ve tabela bilgisini kontrol edip kaydedin.'});
    window.addEventListener('tabela:measurement-error',e=>{live.metricPending=false;live.metricCandidateKey='';const badge=$('liveMetricBadge');if(badge){badge.className='badge bad';badge.textContent='GERÇEK ÖLÇÜ doğrulanmadı • yeniden deneyin'}stage(6,false,e.detail?.message||'Gerçek 3D ölçüm tamamlanmadı')});
    ...
    $('saveBtn')?.addEventListener('click',e=>{const cap=window.TabelaMetric?.capability?.();if(cap?.available&&live.autoMetric&&!currentMeasure?.verified){e.preventDefault();e.stopImmediatePropagation();if($('scanStatus'))$('scanStatus').textContent='Native cihazda kayıt için gerçek 3B ölçümün doğrulanması bekleniyor.'}},true);
  }
  function init(){
  ...
  window.TabelaFastField={startLive,stopLive,resumeLiveSoon,fastScan,requestAutoMetric,state:live,version:'11.2.7-stable-live-roi-single-ocr'};
})();
