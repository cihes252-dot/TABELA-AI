(() => {
  ...
  async function prewarm(onProgress){try{await ensure(onProgress);return true}catch(error){console.warn('Hızlı Türkçe OCR hazırlanamadı',error);return false}}
  function ready(){return !!worker}
  async function run(data,shape,onProgress,timeoutMs=TARGET_MS){
    const started=performance.now(),img=await load(data),w=await ensure(onProgress),roi=cropForOCR(img,shape?.bbox);
    const psm=shape?.shapeType==='vertical-rectangle'?'6':'7';
    ...
    onProgress?.(12,'Türkçe OCR • tek tarama');
    let timer;
    try{
      const recognition=w.recognize(roi);
      const timeout=new Promise((_,reject)=>{timer=setTimeout(()=>reject(new Error('fast_ocr_timeout')),timeoutMs)});
      const r=await Promise.race([recognition,timeout]);clearTimeout(timer);
      const text=clean(r?.data?.text),raw=Math.round(clamp(Number(r?.data?.confidence)||0,0,100)),elapsed=Math.round(performance.now()-started);
      const confidence=Math.round(clamp(raw+(text.length>=4?4:0),0,98)),validated=!!(text.length>=3&&raw>=86);
      onProgress?.(100,`Tek OCR • ${elapsed} ms`);
      return{text,confidence,rawConfidence:raw,validated,consensus:1,consensusFrames:1,candidates:text?[{text,confidence:raw,engine:'tesseract-tur-eng-single'}]:[],singlePass:true,language:'tur+eng',alphabet:'TR+Latin',elapsedMs:elapsed,targetMs:TARGET_MS,targetMet:elapsed<=TARGET_MS,validationMode:validated?'single-pass-high-confidence':'single-pass-user-check',version:'11.2.0-fast-tr-single'};
    }catch(error){
      clearTimeout(timer);const elapsed=Math.round(performance.now()-started);
      if(error?.message==='fast_ocr_timeout'){try{await w.terminate()}catch{}worker=null;workerPromise=null;onProgress?.(100,'OCR 3 sn hedefini aştı • manuel kontrol');return{text:'',confidence:0,rawConfidence:0,validated:false,consensus:0,consensusFrames:0,candidates:[],singlePass:true,timedOut:true,elapsedMs:elapsed,targetMs:TARGET_MS,targetMet:false,validationMode:'fast-timeout-manual',version:'11.2.0-fast-tr-single'}}
      throw error;
    }
  }
  const legacy=window.TabelaOCR||{};
  window.TabelaOCR={...legacy,legacyPrewarm:legacy.prewarm,fastRun:run,prewarm,fastReady:ready,fastTargetMs:TARGET_MS,turkishAlphabet:TURKISH_ALPHABET,version:'11.2.0-fast-tr-single'};
  window.TabelaFastOCR={run,prewarm,ready,targetMs:TARGET_MS,alphabet:TURKISH_ALPHABET,version:'11.2.0-fast-tr-single'};
})();