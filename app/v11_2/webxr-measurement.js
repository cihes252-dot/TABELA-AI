(() => {
  ...
  function planeDeviation(ps){
    ...
    let bestN=null,bestOrigin=null,bestMagnitude=0;
    for(let i=0;i<ps.length-2;i++)for(let j=i+1;j<ps.length-1;j++)for(let k=j+1;k<ps.length;k++){
      const n=cross(sub(ps[j],ps[i]),sub(ps[k],ps[i])),m=norm(n);if(m>bestMagnitude){bestMagnitude=m;bestN=n;bestOrigin=ps[i]}
    }
    if(!bestN||bestMagnitude<1e-8)return Infinity;
    const n=unit(bestN);return Math.max(...ps.map(p=>Math.abs(dot(sub(p,bestOrigin),n))));
  }
  ...
  function validatePlanarity(ps){const dev=planeDeviation(ps),span=maxSpan(ps),limit=Math.max(.025,Math.min(.08,Math.max(span,.1)*.01));if(!Number.isFinite(dev)||dev>limit)throw new Error(`3B noktalar yeterince düz değil (${Number.isFinite(dev)?(dev*100).toFixed(1):'∞'} cm)`);return{dev,limit}}
