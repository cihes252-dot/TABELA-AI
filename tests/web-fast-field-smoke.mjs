...
const required=[
  ...
  [accuracy,'detected-sign-roi'],[accuracy,'headline-fallback-after-full-roi'],[accuracy,'ocrAttempts'],[accuracy,'lowConfidenceRejected'],[accuracy,'confidence>=68'],[accuracy,'Güven 70 altında'],[accuracy,'11.2.10-full-roi-first'],
  ...
  [sw,'tabela-ai-v11-2-11.2.10-ocr-full-roi'],[sw,'./field-accuracy-fix.js']
];
for(const [text,token] of required)if(!text.includes(token))throw new Error('V11.2.10 contract missing: '+token);
...
if(!/let first=await original\(data,shape,onProgress,timeoutMs\)/.test(accuracy))throw new Error('OCR must read the full detected sign ROI first');
...
console.log('V11.2.10 FAST FIELD OCR field fix smoke test OK');